...
const PROTECTED_PATHS = ["/PROTECTED_PATHS", "/api"];  // 所有受保护的路由前缀

{/*
  const PUBLIC_PATHS = ["/", "/about", "/pricing"];
const needsRedirect = !PUBLIC_PATHS.some(p => pathname === p);

// 使用：
} else if (res.status === 401) {  
  setUser(null);
  if (needsRedirect) {          // ← 替换原来的 startsWith("/workspace")
  ...
  const refreshUser = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (pathname?.startsWith("/PROTECTED_PATHS")) {
...
export function useRequireAuth(): AuthContextType {
  ...
  useEffect(() => {
    ...
    if (!auth.isLoading && !auth.isAuthenticated) {
      router.push(buildLoginUrl(pathname || "/PROTECTED_PATHS"));
    }
